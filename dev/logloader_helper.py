...
class log_read_csv():
    ...
    def convert_int_collect_failures(self,row,col,lines):
        ...
        try:
            ...
            self.wrong_lines[row['index']] = lines[row['index']] \
                                             .decode("utf-8").strip()
            ans= pd.np.nan
        return ans
...
#   REGEX PATTERNS OF FIELDS              
    ...
    def generate_fix_suggest(self,index,split):
        ...
        #guessing the 'size' field as 'number', since it is often empty in logs
        for i in range(len(self.fix_suggest)):
            if self.fix_suggest[i]=='status' and self.fix_suggest[i+1]=='empty':
                self.fix_suggest[i+1]='number'
                break
